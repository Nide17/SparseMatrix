...
{
	if (inputObject.matrix->rows != rows)
	{
		char *message = new char[2048];
	...
	if (inputObject.matrix->cols != cols)
	{
		char *message = new char[2048];
	...
	SparseMatrix resultMat(inputObject.matrix->rows, inputObject.matrix->cols);
	int value;

	for (int currRow = 0; currRow < inputObject.matrix->rows; currRow++)
	{
		...
		{
			for (int currCol = 0; currCol < inputObject.matrix->cols; currCol++)
			{
...
SparseMatrix SparseMatrix::operator*(SparseMatrix &inputObject)
{
	if (inputObject.matrix->rows != cols)
	{
		char *message = new char[2048];
	...
	if (inputObject.matrix->cols != rows)
	{
		char *message = new char[2048];
	 ...
	 */
	SparseMatrix resultMat(rows, inputObject.matrix->cols);
	int value;
	...
	// 			for (int rowOf2 = 0; rowOf2 < inputObject.matrix->rows; rowOf2++)
	// 			{
	// 				// printf("Multiplying!\n");
